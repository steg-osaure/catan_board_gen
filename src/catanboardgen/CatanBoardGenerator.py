...
from catanboardgen.Tile import Tile
from catanboardgen.utils import where
...
class CatanBoardGenerator(toga.App):
    ...
    def res_wfc(self):
        ...
        while not all(t.res_collapsed for t in self.tiles):
            ...
            res_idx_list = [
                i for (i, t) in enumerate(self.tiles) if not t.res_collapsed
            ]
            res_opt_list = [
                len(t.res_options)
            ...
            if self.options["Ressource_clusters"]:
                ...
                non_collapsed_neighbours = [
                    t
                    for t in self.tiles
                    if (t.coords in t_col.neighbours() and not t.res_collapsed)
                ]
                for n in non_collapsed_neighbours:
                    ...
                    nb_res_neighbours = len(
                        [
                            ...
                            for t in self.tiles
                            if (
                                (t.coords in n.neighbours())
                                and (t.res_collapsed)
                                and (t.ressource == res_col)
                            )
                        ]
                    )
                    ...
                    if (
                        (res_col in ["wheat", "wood", "sheep"])
                        & (nb_res_neighbours >= 2)
                    ) | (
                        (res_col in ["brick", "stone", "desert"])
                        & (nb_res_neighbours >= 1)
                    ):
            ...
            if any(
                ((len(t.res_options) == 0) & (not t.res_collapsed)) for t in self.tiles
            ):
                return False
    ...
    def num_wfc(self):
        ...
        while not all(t.num_collapsed for t in self.tiles):
            ...
            # pick the tile with the least options (from non-collapsed tiles)
            num_idx_list = [
                i for (i, t) in enumerate(self.tiles) if not t.num_collapsed
            ]
            num_opt_list = [
                len(t.num_options)
                for (i, t) in enumerate(self.tiles)
            ...
            if self.options["Number_clusters"]:
                ...
                non_collapsed_neighbours = [
                    t
                    for t in self.tiles
                    if (t.coords in t_col.neighbours() and not t.num_collapsed)
                ]
                for n in non_collapsed_neighbours:
            ...
            if self.options["Number_repeats"]:
                ...
                non_collapsed_same_res = [
                    t
                    for t in self.tiles
                    if (t.ressource == t_col.ressource and not t.num_collapsed)
                ]
                ...
                if n_col in [6, 8]:
                    ...
                    if not self.options[
                        "More_players"
                    ]:  # or (self.options["More_players"] and ress_has_two68):
                        for n in non_collapsed_same_res:
                            n.num_options = [
                                num for num in n.num_options if num != other_n
                            ]
            ...
            if any(
                ((len(t.num_options) == 0) & (not t.num_collapsed)) for t in self.tiles
            ):
                return False
    ...
    def check_ports(self):
        ...
        for i, p in enumerate(self.ports):
            ...
            same_type_neighbours = [
                t.coords
                for t in self.tiles
                if t.coords in neighbours and t.ressource == r
            ]
