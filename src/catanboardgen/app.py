...
class CatanBoardGenerator(toga.App):
    ...
    def res_wfc(self):
        ...
        while not all([t.res_collapsed for t in self.tiles]):
            ...
            if any(
                [
                    ((len(t.res_options) == 0) & (not t.res_collapsed))
                    for t in self.tiles
                ]
            ):
                return False
    ...
    def num_wfc(self):
        ...
        while not all([t.num_collapsed for t in self.tiles]):
            ...
            if any(
                [
                    ((len(t.num_options) == 0) & (not t.num_collapsed))
                    for t in self.tiles
                ]
            ):
                return False
...
class Tile:
    ...
    def __init__(
        ...
        Args:
            ...
            ressource (str, optional): The type of resource the tile represents. Defaults to "desert".
